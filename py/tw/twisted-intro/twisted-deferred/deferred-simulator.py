...
class Deferred(object):
    ...
    def draw(self, screen, x, y):
        ...
        for callback, errback in self.pairs:
            ...
            errback.draw_passive(
                screen, x + self.callback_width + 2, y, self.callback_width
            )
            y += Callback.height + 3
...
class FiredDeferred(object):
    ...
    def make_drawing_deferred(self, screen, x, y):
        ...
        def draw_connection(x):
            if state.last_x == x:
                screen.draw_vert_line(
                    x - 1 + callback_width / 2, state.last_y - 3, 3, True
                )
                return

            if state.last_x < x:
                screen.draw_vert_line(callback_mid_x, state.last_y - 3, 2)
                screen.draw_vert_line(errback_mid_x, state.last_y - 2, 2, True)
            else:
                screen.draw_vert_line(errback_mid_x, state.last_y - 3, 2)
                screen.draw_vert_line(callback_mid_x, state.last_y - 2, 2, True)

            screen.draw_horiz_line(
                callback_mid_x + 1, state.last_y - 2, errback_mid_x - callback_mid_x - 1
            )
