...
class Task(object):
    ...
    def __init__(self,target):
        ...
        self.stack   = []            # Call stack 一个调用栈

    # Run a task until it hits the next yield statement
    # Task的target会不停的改变，逐次以target试验
    def run(self):
        while True:
            try:
                result = self.target.send(self.sendval)
                if isinstance(result,SystemCall): return result
                if isinstance(result,types.GeneratorType): # 返回的是另一个协程
                    self.stack.append(self.target) # 保存的是当前协程
                    self.sendval = None
                    self.target  = result
                else:
                ...
                self.sendval = None
                self.target = self.stack.pop() # 最近一个协程拿出来
