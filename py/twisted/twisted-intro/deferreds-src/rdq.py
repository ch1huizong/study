# This class (and its test suite) can be found at https://launchpad.net/txrdq
...
class QueueStopped(Exception): pass
...
class ControllableDeferred(defer.Deferred):
    ...
    def cancel(self):
        if not self._called:
            self._called = True
            defer.Deferred.errback(self, Cancelled())
...
class ResizableDispatchQueue(object):
    ...
    def _call(self, obj):
        if not obj is self._nop:
            d = ControllableDeferred(self._func, obj)
            underway = Underway(obj, d)
            self._underway.add(underway)
            def _done(result, item):
                self._underway.remove(item)
                return result
            def _ignoreCancellation(fail):
                fail.trap(Cancelled)
                log.msg('Job %r has been cancelled.' % (obj,))
