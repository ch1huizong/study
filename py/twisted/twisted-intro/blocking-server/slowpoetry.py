...
"""
    ...
    options, args = parser.parse_args()  # 这里解析

    if len(args) != 1:
        parser.error('Provide exactly one poetry file.')  # 提供poetry文件
...
def send_poetry(sock, poetry_file, num_bytes, delay):
    ...
    while True:
        ...
        if not bytes: # no more poetry :( 诗歌内容读完了
            sock.close()
            inputf.close()
            return
        ...
        try:
            sock.sendall(bytes) # this is a blocking call 阻塞操作
        except socket.error:
            sock.close()
            inputf.close()
            return
...
def serve(listen_socket, poetry_file, num_bytes, delay):
    while True:
        ...
        print 'Somebody at %s wants poetry!' % (addr,)  # 客户端连接进来
