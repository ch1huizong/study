...
%%---
%% Row#record{f1=chz, f2=18 } ~ {record, chz, 18}
...
do_this_once() ->
    ...
    mnesia:start(),
    mnesia:create_table(shop,   [{attributes, record_info(fields, shop)}]), % 创建表定义 
...
start() ->
    mnesia:start(),
    mnesia:wait_for_tables([shop,cost,design], 20000).  % 等待表定义载入完成
...
reset_tables() ->
    ...
    F = fun() ->
		foreach(fun mnesia:write/1, example_tables())  %% 写记录实际的元组,不是写记录
	end,
    mnesia:transaction(F).
