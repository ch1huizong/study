...
init([Callback, LSock, UserArgs, Parent]) -> 
    {ok, UserData} = Callback:init(UserArgs), % 第一次交互callback
...
handle_info({http, _Sock, http_eoh},
    ...
    {stop, normal, handle_http_request(State)}; % 1
...
handle_info({tcp, _Sock, Data}, State) when is_binary(Data) ->
    ...
    if 
        ContentRem > 0 ->
            ...
            {noreply, NewState};
        true ->
            {stop, normal, handle_http_request(NewState)} % 2
    end;
