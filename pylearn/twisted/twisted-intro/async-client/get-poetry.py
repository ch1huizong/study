...
def get_poetry(sockets):
    ...
    while sockets:   # 循环sockets
        ...
        rlist, _, _ = select.select(sockets, [], [])   # 阻塞

        # rlist is the list of sockets with data ready to read

        for sock in rlist:  # 每一次的迭代对应一个内部任务
            data = ''

            # 每个client_sock尽最大努力读取数据
            while True:
                try:
            ...
            if not data:
                sockets.remove(sock)   # client_sock已经收完所有的数据
                sock.close()
...
def main():
    ...
    poems = get_poetry(sockets)  # 返回所有的诗歌？ 阻塞
