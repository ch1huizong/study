...
files(Dir, Reg, Recursive, Fun, Acc) ->  %%无论什么情况，都生成一个累积器A
...
find_files([File|T], Dir, Reg, Recursive, Fun, Acc0) ->
	...
	regular ->
		case re:run(FullName, Reg, [{capture,none}]) of
			match  -> 
				...
				find_files(T, Dir, Reg, Recursive, Fun, Acc);  %%更新Acc
			nomatch ->
	...
	directory -> 
	    case Recursive of
		true ->
		    Acc1 = files(FullName, Reg, Recursive, Fun, Acc0), %why? I know!
		    find_files(T, Dir, Reg, Recursive, Fun, Acc1); %%更新Acc
		false ->
		    find_files(T, Dir, Reg, Recursive, Fun, Acc0)
	    end;
	error -> 
